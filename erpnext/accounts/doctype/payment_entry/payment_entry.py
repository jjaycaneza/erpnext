...
def get_orders_to_be_billed(posting_date, party_type, party,
	...
	order_list = []
	for d in orders:
		outstanding_amt_greater_than = filters.get("outstanding_amt_greater_than") if filters.get("outstanding_amt_greater_than") is not None else 0
		outstanding_amt_less_than = filters.get("outstanding_amt_less_than") if filters.get("outstanding_amt_less_than") is not None else 0
		if not (flt(d.outstanding_amount) >= flt(outstanding_amt_greater_than) and flt(d.outstanding_amount) <= flt(outstanding_amt_less_than)):
			continue
