...
class StockEntry(StockController):
	...
	def on_cancel(self):
		...
		if self.work_order and self.purpose == "Material Consumption for Manufacture":
			...
			self.update_work_order()
		lock = ["Receive at Warehouse", "Receive at Branch", "Send to Branch"]
		print("Natawag na sad cancel")
		if self.purpose not in lock:
			print("gawas sa lock")
			self.update_stock_ledger()

		else:
			print("na sulod sa lock cancel")
			if self.purpose == "Send to Branch" and self.from_warehouse == "03L - GAISANO COOP FRESH (OSMENA) - G":
				if len(frappe.get_list("LCV Stock Entry", {"stock_entry": self.name, "docstatus": 2}, ["*"])) != 0:
					print("Naka sulod na siya para mag update cancel")
					from erpnext.stock.get_item_details import get_item_details as g_i_d
					print("WAAAAAAAAAAAAAAAAAAAAAAATCH THIS cance;")
					...
					for item in self.items:
						...
						incoming_outgoing = -1 * (valuation_rate * item.qty)
						frappe.db.sql(
							"UPDATE `tabStock Entry Detail` SET valuation_rate = %s,basic_rate = %s,basic_amount = %s,amount = %s,incoming_outgoing=%s WHERE name = %s",
							(valuation_rate, basic_rate, base_amount, amount, incoming_outgoing, item.name))
					...
					frappe.db.sql(
						"UPDATE `tabStock Entry` SET total_incoming_value = %s,total_outgoing_value=%s,value_difference = %s WHERE name = %s",
						(total_incoming_value, total_outgoing_value, (total_incoming_value - total_outgoing_value),
						 self.name))

					self = frappe.get_doc("Stock Entry", self.name)
					self.docstatus = 2
					self.update_stock_ledger()
				print("Send to branch pero wa na update cancel")
		# self.update_stock_ledger()
